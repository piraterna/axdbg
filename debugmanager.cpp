...
QMultiMap<QString, QVariant> parseKeyVal(const QString& str, QString::const_iterator& it, QChar terminator='\0');
QMultiMap<QString, QVariant> parseDict(const QString& str, QString::const_iterator& it);
...
QMultiMap<QString, QVariant> parseDict(const QString& str, QString::const_iterator& it)
{
    QMultiMap<QString, QVariant> m = parseKeyVal(str, it, '}');
    ++it;
    return m;
}

QMultiMap<QString, QVariant> parseKeyVal(const QString& str, QString::const_iterator& it, QChar terminator)
{
...
QVariant parseValue(const QString& str, QString::const_iterator& it, QChar terminator)
{
    if (*it == '"') {
        ...
        return parseDict(str, ++it).value(str);
    }
    if (mi::_CLOSE_CHARS.contains(*it))
        return {};
    return parseKeyVal(str, it, terminator).values(str);
}

}

QMultiMap<QString, QVariant> parseElements(const QString& str)
{
    auto it = str.cbegin();
...
Response parse_response(const QString& gdb_mi_text)
{
    ...
    if ((m = _GDB_MI_NOTIFY_RE.match(gdb_mi_text)).hasMatch()) {
        return { Response::notify, m.captured(2), parseElements(m.captured(3)).values(), strToInt(m.captured(1), -1) };
    } else if ((m = _GDB_MI_RESULT_RE.match(gdb_mi_text)).hasMatch()) {
        return { Response::result, m.captured(2), parseElements(m.captured(3)).values(), strToInt(m.captured(1), -1) };
    ...
    self(new Priv_t{this})
{
    ...
    connect(self->gdb, &QProcess::finished,
        ...
        if (r.message == "done" || r.message == "") {
            static const QMap<QString, dispatcher_t> doneDispatcher{
                ...
                { "stack", [this](const mi::Response& r) {
                     ...
                     auto stackTrace = r.payload.toMap().value("stack").toList().first().toMap().values();
            ...
            if (self->resposeExpected.contains(r.token)) {
                const auto& e = self->resposeExpected.value(r.token);
                e.handler(r.payload);
...
gdb::Frame gdb::Frame::parseMap(const QMap<QString, QVariant> &data)
{
    gdb::Frame f;
...
gdb::Breakpoint gdb::Breakpoint::parseMap(const QMap<QString, QVariant> &data)
{
    gdb::Breakpoint bp;
...
gdb::Variable gdb::Variable::parseMap(const QMap<QString, QVariant> &data)
{
    gdb::Variable v;
...
gdb::Thread gdb::Thread::parseMap(const QMap<QString, QVariant> &data)
{
    gdb::Thread t;
